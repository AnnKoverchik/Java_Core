...
class Task02 {
    ...
    static double getTotalDepositAmount(double depositAmount, double annualDepositPercent, int depositTerm) {
        ...
        double depositAmountNew=depositAmount;
        for (int i=1; i<=depositTerm; i++) {
            depositAmountNew=depositAmountNew+depositAmountNew*annualDepositPercent/100;
          //  System.out.println(depositAmountNew);
        }
        double totalDepositAmount=Math.round(depositAmountNew*100);

        return totalDepositAmount/100;
    }
}
