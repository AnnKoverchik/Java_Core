...
class Task08 {
    ...
    static String generateTotalPriceList(int startNumberItems, double startPriceAllItems, int differentialNumberItems, double differentialSell, int sizeTotalPrice) {
        ...
        String price=""+startNumberItems+" - "+startPriceAllItems+" with sell 0.0%";
        double nextPriceAllItems=0.00;
        double newDifferentialSell=differentialSell;
        for (int i=(startNumberItems+differentialNumberItems);i<(differentialNumberItems*sizeTotalPrice+startNumberItems); i=i+differentialNumberItems){
            nextPriceAllItems=Math.round((startPriceAllItems/startNumberItems*i-startPriceAllItems/startNumberItems*i*newDifferentialSell/100)*100);
            price=price+"\n"+i+" - "+nextPriceAllItems/100+" with sell "+newDifferentialSell+"%";
            newDifferentialSell=newDifferentialSell+differentialSell;
        }
        return price;
    }
}
